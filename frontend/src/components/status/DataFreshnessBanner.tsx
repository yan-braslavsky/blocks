...
export function DataFreshnessBanner({ lastIngestAt, className = '' }: DataFreshnessBannerProps) {
  if (!lastIngestAt) {
    return (
  ...
  const borderColor = status === 'warning' ? 'border-yellow-200' : 'border-red-200';
  const textColor = status === 'warning' ? 'text-yellow-800' : 'text-red-800';
  const descriptionColor = status === 'warning' ? 'text-yellow-700' : 'text-red-700';
    ...
    <div className={`${bgColor} border ${borderColor} rounded-md p-4 ${className}`}>
      <div className="flex">
        ...
        <div className="ml-3">
          ...
          <p className={`mt-1 text-sm ${descriptionColor}`}>
            {description}
          </p>
        </div>
      </div>
