...
/**
 * Timeline block structure (types will be imported from shared later)
 */
export interface TimelineBlock {
  id: string;
  title: string;
  metricType: 'Spend' | 'Performance' | 'Projection' | 'Other';
  timeRange: string;
  ...
  for (let i = 29; i >= 0; i--) {
    ...
    switch (metricType) {
      case 'Spend':
        ...
        value = Math.max(100, baseSpend * weeklyMultiplier + random.nextFloat(-100, 100));
        break;
        
      case 'Performance':
        // Performance data: 0-100% with gradual trends
        const basePerf = 75 + Math.sin((i / 30) * Math.PI * 2) * 15;
        value = Math.max(0, Math.min(100, basePerf + random.nextFloat(-10, 10)));
        break;
      ...
      default:
        ...
        const previousValue = dataPoints.length > 0 ? dataPoints[dataPoints.length - 1]?.value ?? 500 : 500;
 ...
 */
export function generateMockTimelines(date: Date = new Date()): TimelineBlock[] {
  ...
  const shuffledTemplates = [...TIMELINE_TEMPLATES].sort(() => random.next() - 0.5);
 ...
 */
export function getMockTimelinesResponse(date: Date = new Date()) {
  return {
...
export function generateSingleTimeline(
  ...
  const random = createDailyRandom(date);
  const template = TIMELINE_TEMPLATES.find(t => t.metricType === metricType) || TIMELINE_TEMPLATES[0];
