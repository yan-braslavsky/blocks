...
/**
 * Basic recommendation stub structure (types will be imported from shared later)
 */
export interface RecommendationStub {
  ...
  status: 'Prototype' | 'ComingSoon' | 'Future';
  category?: string;
  displayOrder?: number;
...
const RECOMMENDATION_TEMPLATES: RecommendationStub[] = [
  {
    ...
    shortDescription: 'Identify over-provisioned instances running below optimal capacity',
    impactLevel: 'High',
    status: 'Prototype',
    category: 'cost',
    displayOrder: 1,
  ...
  },
  {
    ...
    shortDescription: 'Migrate suitable workloads to cost-effective spot instances',
    impactLevel: 'Medium',
    status: 'ComingSoon',
    category: 'cost',
    displayOrder: 4,
  ...
  },
  {
    ...
    shortDescription: 'Transition infrequently accessed data to cheaper storage tiers',
    impactLevel: 'Medium',
    status: 'Prototype',
    category: 'storage',
    displayOrder: 5,
  ...
  },
  {
    ...
    shortDescription: 'Right-size Lambda functions for optimal cost-performance ratio',
    impactLevel: 'Low',
    status: 'Future',
    category: 'serverless',
    displayOrder: 6,
  ...
  },
  {
    ...
    shortDescription: 'Optimize database instance sizes based on actual usage patterns',
    impactLevel: 'High',
    status: 'ComingSoon',
    category: 'database',
    displayOrder: 7,
 ...
 */
function deduplicateRecommendations(recommendations: RecommendationStub[]): RecommendationStub[] {
  ...
  if (duplicateCount > 0 && process.env.NODE_ENV === 'development') {
    console.log(`[Mock Recommendations] Removed ${duplicateCount} duplicate recommendation(s)`);
  }
 ...
 */
export function generateMockRecommendations(date: Date = new Date()): RecommendationStub[] {
  ...
  const shuffledTemplates = [...RECOMMENDATION_TEMPLATES].sort(() => random.next() - 0.5);
  
  for (let i = 0; i < Math.min(numRecommendations, shuffledTemplates.length); i++) {
  ...
  while (selectedTemplates.length < numRecommendations) {
    const templateIndex = selectedTemplates.length % RECOMMENDATION_TEMPLATES.length;
  ...
  return deduplicated.sort((a, b) => (a.displayOrder || 0) - (b.displayOrder || 0));
}

/**
 * Get mock recommendations in API response format
 */
export function getMockRecommendationsResponse(date: Date = new Date()) {
  return {
